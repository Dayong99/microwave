...
<!-- eslint-disable -->
  <div class="charts">
    ...
    <el-button @click="getSearchSkyData">探空对比</el-button>
    <el-button @click="getMicwareData">微波对比</el-button>
...
export default {
  ...
  methods: {
    // !画图表
    drawLine(SearchSkyArray, micArray,probeArray) {
      // !通过ref获取到DOM节点
      if (myChart) {
    ...
    getSearchSkyData() {
      ...
      this.$get(this.$store.state.FY4.fyProbeRatioList, {
        ...
        .then(res => {
          // !探空雷达二维数组 0 为x轴(温度) 1 为 y轴(高度)
          const SearchSkyArray = [];
          // !为探空雷达二维数组赋值
          // !对温度进行筛选 保留大于200 小于1000的温度
          let a1 = [],
            b1 = [];
          res.data.data.探空.data.TEMP.forEach((item, index) => {
            if (item > -200 && item < 1000) {
              // !有效温度
              a1.push(item);
              // !温度对应高度
              b1.push(res.data.data.探空.data.HEIGHT[index]);
            }
          });
          // !对高度进行筛选 删除40以下
          const s_T = [],
            s_H = [];
          b1.forEach((item, index) => {
            if (item > 100) {
              // !筛选高度
              s_H.push(item);
              // !高度对应温度
          ...
          this.drawLine(SearchSkyArray,null, probeArray);
        })
        .catch(res => {
          this.$notify.error({
            title: "error",
    ...
    closeChart() {
      this.$store.commit('Charts/setIsEchartsProbeShow', false)
    }
  }
};
</script>
